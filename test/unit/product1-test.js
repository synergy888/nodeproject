...
describe('Test Prompts', function(){
	...
	describe('Mobile Commerce tests for new customer', function(){
		...
		describe('Commerce version', function(){
			it('should match commerce version prompt text (Is this for MobileCommerce 1.2 or MobileCommerce 1.3?)', function(){
				expect(prompts.commerce_version.trim()).equal('Is this for MobileCommerce 1.2 or MobileCommerce 1.3?'.trim());
			});

			it('should return 1.3', function(){
				expect(transformProperties.companyObj.products_commerce_version).equal('1.3');
			});
		});
